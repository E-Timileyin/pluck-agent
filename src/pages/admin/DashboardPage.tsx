 ...
 */
export function DashboardPage(props: { admin: Admin; stats: DashboardStats; rows: AttemptRow[] }) {
  const { stats } = props;
  const unfinished = stats.attempts - stats.completed;
  const failed = stats.completed - Math.round((stats.completed * (stats.passRate ?? 0)) / 100);
    ...
    <AdminShell title="Overview" active="overview" admin={props.admin} bare>
      ...
      <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        <Panel class="flex h-full flex-col lg:col-span-2">
          <PanelHead Icon={FiActivity} title="Attempts" aside={<Chip tone="quiet">All time</Chip>} />
            ...
            <div class="min-w-0">
              ...
              <p class="m-0 mt-2 flex flex-wrap items-center gap-2 text-[13px] text-muted">
                <Chip tone="good" Icon={FiUsers}>
                  {stats.uniquePromoters} sales {stats.uniquePromoters === 1 ? 'agent' : 'agents'}
                </Chip>
                {stats.completed} completed · {unfinished} unfinished
              </p>
            </div>

            <ol class="m-0 flex list-none items-end gap-2 p-0" aria-hidden="true">
              {segments.map((segment) => (
                <li class="flex w-10 flex-col items-center gap-1.5">
                  <span class="text-[11px] font-semibold text-muted">{segment.value}</span>
                  <span
          ...
          <div class="mt-5 flex flex-wrap gap-2">
            <a
              class="inline-flex min-h-11 items-center gap-2 rounded-full bg-brand px-5 text-sm font-semibold text-white no-underline transition-colors duration-150 hover:bg-brand-deep"
            ...
            <a
              class="inline-flex min-h-11 items-center gap-2 rounded-full bg-ink px-5 text-sm font-semibold text-white no-underline"
            ...
            <a
              class="inline-flex min-h-11 items-center gap-2 rounded-full border border-line bg-white px-5 text-sm font-semibold text-ink no-underline transition-colors duration-150 hover:border-brand hover:text-brand"
              href="/admin/settings"
            >
              <FiSettings size={17} />
              Settings
        ...
        >
          <span class="flex h-2.5 overflow-hidden rounded-full bg-line" aria-hidden="true">
            {segments.map((segment) => (
              <span
          ...
          <ul class="m-0 mt-3 flex list-none flex-wrap gap-x-4 gap-y-1 p-0 text-[11px] text-muted">
            {segments.map((segment) => (
              <li class="flex items-center gap-1.5">
                <span class={`size-2 rounded-full ${segment.colour}`} aria-hidden="true"></span>
                {segment.label}
              </li>
            ))}
          </ul>
        ...
        >
          <p class="m-0 text-[13px] text-muted">
            ...
            {stats.formatSplit.unset === 1 ? 'attempt has' : 'attempts have'} not opened either yet.
          </p>
        </FigureTile>
        ...
        <Panel class="h-full lg:col-span-2">
          <PanelHead
            ...
            aside={
              <a
                class="inline-flex items-center gap-1.5 text-sm font-semibold text-brand no-underline hover:underline"
                href="/admin/attempts"
              >
                See all
          ...
          {props.rows.length === 0 ? (
            <p class="m-0 text-[15px] text-muted">
              No attempts yet. Share the training link with a sales agent and the first one lands
              here.
            </p>
          ) : (
                  ...
                  >
                    <span class="min-w-0 flex-1">
                      ...
                      <span class="block text-[11px] text-muted">
                        {promoter.tier} · {attempt.tutorialMode ?? 'no format yet'}
                      </span>
                    </span>

                    <span class="shrink-0 text-sm font-semibold text-ink">
                      {attempt.submittedAt ? `${attempt.score}/${attempt.total}` : '—'}
                    </span>
        ...
        <Panel class="h-full">
          ...
          {stats.mostMissed ? (
            <div class="rounded-2xl bg-brand-deep p-4">
              <p class="m-0 line-clamp-3 text-sm/[1.5] font-semibold text-white">
                {stats.mostMissed.prompt}
              </p>
              <p class="m-0 mt-3 flex items-baseline gap-2">
                <span class="text-[30px] leading-none font-bold text-white">
                  {Math.round((stats.mostMissed.missed / Math.max(1, stats.mostMissed.asked)) * 100)}
                  %
                </span>
            ...
            <p class="m-0 text-[15px] text-muted">
              Nothing has been missed yet. This is the one tile that says something about the
              training material rather than the people taking it.
            </p>
          )}
        </Panel>

        <div class="grid content-start gap-3">
          <MiniTile Icon={FiUsers} label="Directory" title="Sales agents" href="/admin/promoters" />
          <MiniTile
            Icon={FiUpload}
