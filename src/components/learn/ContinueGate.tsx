...
/**
 * The bar under the overview: one sentence about where you are, and the button
 * out of this screen.
 *
 * The countdown here is display only — the gate itself is checked server-side
 * in POST /learn/continue. A disabled button is defeated by devtools in five
 * seconds, so the button being enabled proves nothing.
 */
export function ContinueGate(props: { remainingSeconds: number }) {
  ...
  return (
    <div class="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-line bg-white p-3.5">
      ...
      <form method="post" action="/learn/continue" class="shrink-0">
        <button
