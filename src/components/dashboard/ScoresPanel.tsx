    ...
    <Panel class="flex flex-col">
      <div class="flex items-center justify-between gap-3">
        <div class="flex items-center gap-3">
          ...
          <h2 class="m-0 text-base font-semibold text-ink">Your scores</h2>
        </div>

        <button
          type="button"
          class="inline-flex items-center gap-1.5 rounded-full border border-line bg-white px-3 py-1.5 text-[13px] font-semibold text-ink"
        >
          Latest
      ...
      {props.bars.length === 0 ? (
        <div class="flex flex-1 flex-col items-center justify-center gap-3 py-10 text-center">
          ...
          <p class="m-0 text-sm font-semibold text-ink">Nothing scored yet</p>
          <p class="m-0 text-[13px] text-muted">Your first result appears here as a column.</p>
        </div>
      ) : (
