...
--
-- A KNOWN credential, committed on purpose so a fresh database can be signed
-- into without the first-run setup screen. It is a demo account, not a secret:
-- anyone reading this repo can sign in with it.
--
-- Before this is used by real promoters, either delete the row and let
-- /admin/setup create the first real account, or sign in and add a named
-- account from Settings → Team, then:
--
--   npx wrangler d1 execute pluck-training --remote \
--     --command "DELETE FROM admins WHERE email = 'admin@pluck.ng'"
--
-- The hash is PBKDF2-SHA256, 210,000 iterations, generated by lib/password.ts's
-- own format — the app never stores anything else.
-- ---------------------------------------------------------------------------
INSERT OR REPLACE INTO admins (id, name, email, password_hash, created_at, last_login_at)
VALUES (
  'admin-demo',
  'Pluck Admin',
  'admin@pluck.ng',
  'pbkdf2-sha256$210000$qbNyokHAiv+VzDckMk7zAg==$YsbrRPME/3SxuqXQE1dWHoqAzcoALuvRrXDjGzVkoMg=',
  datetime('now'),
  NULL
);
